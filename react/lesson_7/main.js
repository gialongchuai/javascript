...
// https://api-gateway.fullstack.edu.vn/api/blog-posts/featured
...
function PostItem({ // lưu ý là truyền theo destructuring thì cái tên quan trọng 
    // nhé truyền nhầm là toang ngay
...
const jsx = ( // react.development.js:245 Warning: Each child in a list should have a unique "key" prop.
    // cách khắc phục thêm id vào cho các child đã làm trong past
    // cho những child có thêm key là index của nó trong mảng
    <div id="wrapper"> 
        {
            posts.map((post, index) => { // nhớ return trả về nhé
                return <PostItem props={post} key={index} />
            })
        }
    </div>
)
